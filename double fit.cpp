...
const double FIT_MIN = 1.0; // Fit range min for original plot (µs)
const double FIT_MAX = 16.0; // Fit range max for original plot (µs)
const double FIT_MIN_LATE = 10.0; // Fit range min for late plot (µs)
const double FIT_MAX_LATE = 20.0; // Fit range max for late plot (µs)
...
int main(int argc, char *argv[]) {
    ...
    TH1D* h_dt_michel_late = new TH1D("DeltaT_Late", "Late Muon-Michel Time Difference;Time to Previous event(Muon)(#mus);Counts/0.1 #mus", 100, 10.0, 20.0);
    ...
    for (const auto& inputFileName : inputFiles) {
        ...
        for (int iEnt = 0; iEnt < numEntries; iEnt++) {
            ...
            if (p.energy >= MICHEL_ENERGY_MIN && p.energy <= MICHEL_ENERGY_MAX &&
                ...
                h_dt_michel->Fill(dt);
                if (dt >= 10.0 && dt <= 20.0) h_dt_michel_late->Fill(dt);
    ...
    // Original Michel dt with exponential fit
    c->Clear();
    ...
    if (h_dt_michel->GetEntries() > 50) {
        ...
        for (int i = bin_14; i <= bin_16; i++) {
            ...
            if (content > 0) { sum += content; n++; }
        }
        C_init = n > 0 ? sum / n : 0.1;
        ...
        cout << "Exponential Fit Results (Michel dt, 1.0-16.0 µs):\n";
    ...
    // Late Michel dt (10-20 µs) with exponential fit
    c->Clear();
    h_dt_michel_late->SetMarkerStyle(20);
    h_dt_michel_late->SetMarkerSize(1.0);
    h_dt_michel_late->GetXaxis()->SetTitle("Time to Previous Event (Muon) (#mus)");
    h_dt_michel_late->Draw("PE");

    if (h_dt_michel_late->GetEntries() > 50) {
        double bin_width = h_dt_michel_late->GetBinWidth(1);
        double integral = h_dt_michel_late->Integral(h_dt_michel_late->FindBin(FIT_MIN_LATE), h_dt_michel_late->FindBin(FIT_MAX_LATE));
        double N0_init = integral * bin_width / (FIT_MAX_LATE - FIT_MIN_LATE);
        double C_init = 0;
        int bin_18 = h_dt_michel_late->FindBin(18.0);
        int bin_20 = h_dt_michel_late->FindBin(20.0);
        double sum = 0;
        int n = 0;
        for (int i = bin_18; i <= bin_20; i++) {
            double content = h_dt_michel_late->GetBinContent(i);
            if (content > 0) { sum += content; n++; }
        }
        C_init = n > 0 ? sum / n : 0.1;

        TF1 *expFit = new TF1("expFit", ExpFit, FIT_MIN_LATE, FIT_MAX_LATE, 3);
        expFit->SetParameters(N0_init, 2.2, C_init);
        expFit->SetParLimits(0, 0, N0_init * 100);
        expFit->SetParLimits(1, 0.5, 5.0);
        expFit->SetParLimits(2, -C_init * 10, C_init * 10);
        ...
        int fitStatus = h_dt_michel_late->Fit(expFit, "RE", "", FIT_MIN_LATE, FIT_MAX_LATE);
        expFit->SetLineColor(kGreen);
        expFit->SetLineWidth(3);
        expFit->Draw("same");
        gPad->Update();

        TPaveStats *stats = (TPaveStats*)h_dt_michel_late->FindObject("stats");
        if (!stats) {
            stats = new TPaveStats(0.60, 0.60, 0.90, 0.90, "brNDC");
            stats->SetName("stats");
            h_dt_michel_late->GetListOfFunctions()->Add(stats);
        }
        stats->SetTextColor(kRed);
        stats->SetX1NDC(0.60);
        stats->SetX2NDC(0.90);
        ...
        cout << "Exponential Fit Results (Michel dt, 10.0-20.0 µs):\n";
        cout << Form("Fit Status: %d (0 = success)", fitStatus) << endl;
        cout << Form("N₀ = %.1f ± %.1f", N0, N0_err) << endl;
        cout << Form("τ = %.4f ± %.4f µs", tau, tau_err) << endl;
        cout << Form("C = %.1f ± %.1f", C, C_err) << endl;
        ...
        if (fitStatus != 0) {
            cout << "Warning: Exponential fit failed for h_dt_michel_late (status = " << fitStatus << ")" << endl;
            cout << "Initial Parameters: N0 = " << N0_init << ", τ = 2.2 µs, C = " << C_init << endl;
        }
        delete expFit;
    } else {
        cout << "Warning: h_dt_michel_late has insufficient entries (" << h_dt_michel_late->GetEntries() << "), skipping exponential fit" << endl;
    }

    c->Update();
    c->Modified();
    c->RedrawAxis();
    plotName = OUTPUT_DIR + "/Michel_dt_late.png";
    c->SaveAs(plotName.c_str());
    cout << "Saved plot: " << plotName << endl;
