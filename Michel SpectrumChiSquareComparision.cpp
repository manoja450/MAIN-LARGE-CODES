...
void analyzeMuonMichel(TChain *analysisChain, const Double_t *mu1, const string &outputDir) {
    ...
    // Normalize accidental spectrum to same area as signal spectrum in their respective time windows
    double scaleFactor = (10.0-1.0)/(20.0-10.0);
    histMichelSpectrum_Accidental->Scale(scaleFactor);
...
// Add main title (now only one title)
