...
//SiPM4 need to check again
//const double SIPM_THRESHOLDS[N_SIPMS] = {300,200,200,200,700,700,800,700,500,400};
const double SIPM_THRESHOLDS[N_SIPMS] = {50,50,50,50,50,50,50,50,50,50};
// Thresholds for PMTs in ADC counts
const double PMT_THRESHOLDS[N_PMTS] = {40,40,40,40,40,40,40,40,40,40,40,40};
...
void analyzeMuonMichel(TChain *analysisChain, const Double_t *mu1, const string &outputDir) {
    ...
    TH1F *histDeltaT = new TH1F("DeltaT",
        "Muon-Michel Time Difference;Time to previous event(Muon)(#mus);Counts/0.08 #mus",
        200, 0, 16.0);
    
    TH1F *histMichelSpectrum = new TH1F("MichelSpectrum",
        "Michel Electron Energy;Photoelectrons;Counts/4 p.e.", 200, 0, 800);
    
    TH1F *histMichelSpectrum_Accidental = new TH1F("MichelSpectrum_Accidental",
        "Michel Electron Energy (Accidental);Photoelectrons;Counts/4 p.e.", 100, 0, 800);
    
    TH1F *histMuonPMTHits = new TH1F("MuonPMTHits",
        "PMT Multiplicity;Number of PMTs hit;Counts/bin", 13, 0, 13);
    ...
    for (Long64_t entry = 0; entry < nEntries; entry++) {
        ...
        if (sipmHitCount < 1 || sipmHitCount > 10) continue;
    ...
    legEnergy->AddEntry(histMichelSpectrum, "1-16 #mus", "l");
    legEnergy->AddEntry(histMichelSpectrum_Accidental, "16-20 #mus", "l");
